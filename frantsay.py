...
# =============================================================================
# 5. ÉTAT DE SESSION + AUTHENTIFICATION CLOUD
# =============================================================================
...
def _secret(name: str, default: str = "") -> str:
    """Read a secret from Streamlit's server-side secrets."""
    try:
...
def get_supabase_client() -> Client:
    """Create one server-side Supabase client using the secret service-role key."""
    url = _secret("SUPABASE_URL")
    key = _secret("SUPABASE_SERVICE_ROLE_KEY")
    if not url or not key:
        raise RuntimeError(
            "Configuration Supabase manquante : renseigne SUPABASE_URL et "
            "SUPABASE_SERVICE_ROLE_KEY dans .streamlit/secrets.toml."
        )
...
def _otp_digest(email: str, pseudo: str, code: str) -> str:
    secret = _secret("OTP_SECRET") or _secret("SUPABASE_SERVICE_ROLE_KEY")
    if not secret:
        raise RuntimeError("OTP_SECRET manquant dans st.secrets.")
    payload = f"{normalize_email(email)}|{normalize_pseudo(pseudo)}|{code}".encode("utf-8")
    return hmac.new(secret.encode("utf-8"), payload, hashlib.sha256).hexdigest()


def _send_otp_email(email: str, pseudo: str, code: str) -> None:
    api_key = _secret("RESEND_API_KEY")
    from_email = _secret("RESEND_FROM") or "FRANTSAY <onboarding@resend.dev>"
    if not api_key:
        raise RuntimeError("RESEND_API_KEY manquante dans st.secrets.")

    resend.api_key = api_key
    html_body = f"""
    <div style=\"font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:24px\">
      <h2 style=\"margin-bottom:8px\">FRANTSAY — Vérification</h2>
      <p>Bonjour <strong>{html.escape(pseudo)}</strong>,</p>
      <p>Ton code de validation est :</p>
      <div style=\"font-size:32px;font-weight:700;letter-spacing:8px;padding:16px 0\">{code}</div>
      <p>Ce code expire dans {OTP_TTL_MINUTES} minutes. Ne le partage avec personne.</p>
    </div>
    """
    params: resend.Emails.SendParams = {
        "from": from_email,
        "to": [email],
        "subject": "FRANTSAY — Ton code de vérification",
        "html": html_body,
    }
...
def _find_existing_identity(email: str, pseudo: str) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    client = get_supabase_client()
    email_row = _extract_first_row(
        client.table("users").select("id,email,pseudo").eq("email", normalize_email(email)).limit(1).execute()
    )
    pseudo_row = _extract_first_row(
        client.table("users").select("id,email,pseudo").eq("pseudo", normalize_pseudo(pseudo)).limit(1).execute()
    )
    return email_row, pseudo_row


def request_registration_otp(email: str, pseudo: str) -> None:
    """Check uniqueness, create a short-lived OTP and email it."""
    email = normalize_email(email)
    pseudo = normalize_pseudo(pseudo)
    email_row, pseudo_row = _find_existing_identity(email, pseudo)

    if email_row and email_row.get("pseudo") != pseudo:
        raise ValueError("Cet e-mail est déjà associé à un autre pseudo.")
    if pseudo_row and pseudo_row.get("email") != email:
        raise ValueError("Ce pseudo est déjà utilisé.")
    if email_row and pseudo_row:
        raise ValueError("Ce compte existe déjà. Utilise l'écran de connexion.")

    client = get_supabase_client()
    pending = _extract_first_row(
        client.table("pending_otps")
        .select("email,sent_at")
        .eq("email", email)
        .limit(1)
        .execute()
    )
    if pending and pending.get("sent_at"):
        try:
            ...
            if elapsed < OTP_RESEND_COOLDOWN_SECONDS:
                ...
                raise ValueError(f"Attends encore {wait} s avant de redemander un code.")
        except ValueError:
            raise
        except Exception:
            pass
    ...
    row = {
        "email": email,
        "pseudo": pseudo,
        "otp_hash": _otp_digest(email, pseudo, code),
        "expires_at": expires_at.isoformat(),
        "attempts": 0,
        "sent_at": datetime.now(timezone.utc).isoformat(),
    }
    client.table("pending_otps").upsert(row, on_conflict="email").execute()

    try:
        _send_otp_email(email, pseudo, code)
    except Exception:
        client.table("pending_otps").delete().eq("email", email).execute()
        raise

    st.session_state.otp_pending = True
    st.session_state.otp_email = email
    st.session_state.otp_pseudo = pseudo
    st.session_state.otp_sent_at = datetime.now(timezone.utc).isoformat()


def verify_registration_otp(email: str, pseudo: str, code: str) -> dict[str, Any]:
    """Verify OTP, create an immutable UUID v4 account, and return the user."""
    email = normalize_email(email)
    pseudo = normalize_pseudo(pseudo)
    ...
    client = get_supabase_client()
    pending = _extract_first_row(
        client.table("pending_otps").select("*").eq("email", email).limit(1).execute()
    )
    if not pending:
        raise ValueError("Aucun code actif pour cet e-mail. Demande un nouveau code.")
    if normalize_pseudo(str(pending.get("pseudo", ""))) != pseudo:
        raise ValueError("Le pseudo ne correspond pas au code en attente.")

    try:
        expires_at = datetime.fromisoformat(str(pending["expires_at"]).replace("Z", "+00:00"))
    except Exception as exc:
        raise ValueError("Le code OTP est invalide ou expiré.") from exc

    if datetime.now(timezone.utc) > expires_at:
        client.table("pending_otps").delete().eq("email", email).execute()
        raise ValueError("Le code OTP a expiré. Demande un nouveau code.")
    ...
    expected = str(pending.get("otp_hash", ""))
    if not hmac.compare_digest(_otp_digest(email, pseudo, code), expected):
        client.table("pending_otps").update({"attempts": attempts + 1}).eq("email", email).execute()
        remaining = max(0, OTP_MAX_ATTEMPTS - attempts - 1)
        raise ValueError(f"Code incorrect. Tentatives restantes : {remaining}.")

    # Final uniqueness check: the UNIQUE constraints in PostgreSQL remain the source of truth.
    email_row, pseudo_row = _find_existing_identity(email, pseudo)
    if email_row or pseudo_row:
        client.table("pending_otps").delete().eq("email", email).execute()
        raise ValueError("Ce compte vient d'être créé ou le pseudo est déjà utilisé.")

    user_id = str(uuid.uuid4())
    progress = {"score": 0, "questions_done": 0, "level": "Lycée"}
    try:
        response = (
            client.table("users")
            .insert(
            ...
            .select("id,email,pseudo,level,score,questions_done,progress")
            .execute()
        )
    except Exception as exc:
        # PostgreSQL UNIQUE constraints protect the 1:1:1 relationship under races.
        raise ValueError("Impossible de créer le compte : l'e-mail ou le pseudo est déjà utilisé.") from exc
    finally:
        client.table("pending_otps").delete().eq("email", email).execute()

    user = _extract_first_row(response)
    if not user:
        raise RuntimeError("Le compte a été créé mais aucune donnée utilisateur n'a été retournée.")
    return user


def login_by_pseudo(pseudo: str) -> dict[str, Any]:
    pseudo = normalize_pseudo(pseudo)
    if not pseudo:
        raise ValueError("Saisis ton pseudo.")
    client = get_supabase_client()
    ...
    if not user:
        raise ValueError("Pseudo introuvable. Vérifie l'orthographe ou crée un compte.")
    return user


def restore_user_to_session(user: dict[str, Any]) -> None:
    progress = user.get("progress") or {}
    level = progress.get("level") or user.get("level") or "Lycée"
    if level not in LEVELS:
        level = "Lycée"

    st.session_state.user_id = str(user["id"])
    st.session_state.user_email = str(user["email"])
    st.session_state.user_pseudo = str(user["pseudo"])
    st.session_state.level = level
    st.session_state.score = int(progress.get("score", user.get("score", 0)) or 0)
    st.session_state.questions_done = int(
        progress.get("questions_done", user.get("questions_done", 0)) or 0
    )
    ...
    st.session_state.otp_pseudo = None
...
def save_progress(user_id: str, data: dict[str, Any]) -> None:
    """Persist the current progression remotely in Supabase."""
    if not user_id:
        raise ValueError("user_id manquant.")
    payload = {
        "score": int(data.get("score", 0)),
        "questions_done": int(data.get("questions_done", 0)),
        "level": data.get("level", "Lycée") if data.get("level") in LEVELS else "Lycée",
    }
    client = get_supabase_client()
    client.table("users").update(
        {
            "level": payload["level"],
            "score": payload["score"],
            "questions_done": payload["questions_done"],
            "progress": payload,
...
def save_current_progress() -> None:
    """Best-effort save used after every completed activity/level change."""
    ...
    try:
        ...
        st.warning(f"Progression locale mise à jour, mais synchronisation cloud impossible : {exc}")
...
<p class="auth-hero-sub">Ta progression est maintenant sauvegardée dans le cloud.</p></div></div>""",
    ...
    if not supabase_ready():
        st.error("Supabase n'est pas configuré. Vérifie `.streamlit/secrets.toml`.")
        st.stop()

    col1, col2 = st.columns(2)
    with col1:
        if st.button("Créer un compte", key="auth_register_tab", use_container_width=True):
            st.session_state.auth_view = "register"
            st.rerun()
    with col2:
        if st.button("Se connecter", key="auth_login_tab", use_container_width=True):
    ...
    if st.session_state.auth_view == "register":
        ...
        st.markdown(
            '<span class="eyebrow">Créer mon compte</span>'
            '<p style="color:var(--muted);font-size:.8rem;margin:.3rem 0 .8rem 0">'
            "E-mail + pseudo, puis un code OTP envoyé par e-mail pour valider la création."
            "</p>",
            unsafe_allow_html=True,
        )
        ...
        if not st.session_state.otp_pending:
            with st.form("registration_request_form"):
                email_input = st.text_input("E-mail", autocomplete="email")
                pseudo_input = st.text_input("Pseudo", help="3 à 24 caractères : lettres, chiffres, _ . -")
                submitted = st.form_submit_button("Envoyer le code", use_container_width=True)
            if submitted:
                email_clean = normalize_email(email_input or "")
                pseudo_clean = normalize_pseudo(pseudo_input or "")
                if not is_valid_email(email_clean):
                    st.error("Merci de saisir une adresse e-mail valide.")
                elif not is_valid_pseudo(pseudo_clean):
                    st.error("Pseudo invalide : 3 à 24 caractères, lettres/chiffres/_/.- uniquement.")
                else:
                    try:
                        with st.spinner("Envoi du code..."):
                            request_registration_otp(email_clean, pseudo_clean)
                        st.success(f"Code envoyé à {email_clean}.")
                        st.rerun()
                    except Exception as exc:
                        st.error(str(exc))
        else:
            st.info(f"Code envoyé à {st.session_state.otp_email}. Il reste valable {OTP_TTL_MINUTES} minutes.")
            with st.form("registration_verify_form"):
                otp_input = st.text_input("Code OTP à 6 chiffres", max_chars=6, autocomplete="one-time-code")
                verified = st.form_submit_button("Valider et créer mon compte", use_container_width=True)
            if verified:
                try:
                    with st.spinner("Validation du compte..."):
                        user = verify_registration_otp(
                            st.session_state.otp_email,
                            st.session_state.otp_pseudo,
                            otp_input,
                        )
                    restore_user_to_session(user)
                    st.success("Compte créé avec succès.")
                    st.rerun()
                except Exception as exc:
                    st.error(str(exc))
            if st.button("Annuler et recommencer", key="cancel_registration", use_container_width=True):
                st.session_state.otp_pending = False
                st.session_state.otp_email = None
                st.session_state.otp_pseudo = None
                st.session_state.otp_sent_at = None
        ...
        st.markdown(
            ...
            '<p style="color:var(--muted);font-size:.8rem;margin:.3rem 0 .8rem 0">'
            "Entre uniquement ton pseudo pour retrouver ton compte et ta progression cloud."
            "</p>",
            unsafe_allow_html=True,
        )
        with st.form("login_form"):
            pseudo_input = st.text_input("Pseudo", autocomplete="username")
            submitted = st.form_submit_button("Se connecter", use_container_width=True)
        if submitted:
            try:
                with st.spinner("Connexion..."):
                    user = login_by_pseudo(pseudo_input or "")
                restore_user_to_session(user)
                st.rerun()
            except Exception as exc:
                st.error(str(exc))
    ...
    st.caption("Important : comme demandé, le pseudo est la clé d'accès. Toute personne qui connaît le pseudo peut se connecter.")
    st.stop()
...
# =============================================================================
# 5bis. SAUVEGARDE INITIALE ET CONFIGURATION DE LA SESSION
# =============================================================================

# Après une connexion/création de compte, la progression a déjà été restaurée
# depuis Supabase dans st.session_state. Les modifications suivantes sont
# enregistrées à chaque étape via save_current_progress().
