...
# =============================================================================
# 5. ÉTAT DE SESSION + IDENTITÉ FRANTSAY
# =============================================================================
...
def _secret(name: str, default: str = "") -> str:
    """Lit une configuration depuis les secrets serveur Streamlit."""
    try:
...
def get_supabase_client() -> Client:
    """Client Supabase serveur. La clé service-role ne quitte jamais le serveur."""
    url = _secret("SUPABASE_URL")
    key = _secret("SUPABASE_SERVICE_ROLE_KEY")
    if not url or not key:
        raise RuntimeError("Configuration Supabase manquante dans les Secrets Streamlit.")
...
def find_user_by_pseudo(pseudo: str) -> dict[str, Any] | None:
    client = get_supabase_client()
    return _extract_first_row(
        client.table("users")
        .select("id,email,pseudo,level,score,questions_done,progress")
        .eq("pseudo", normalize_pseudo(pseudo))
        .limit(1)
        .execute()
    )


def create_or_open_account(email: str, pseudo: str) -> dict[str, Any]:
    """Crée le profil 1 e-mail = 1 pseudo = 1 UUID, ou retrouve le profil existant."""
    ...
    if existing_email:
        if normalize_pseudo(str(existing_email.get("pseudo", ""))) != pseudo:
            raise ValueError("Cet e-mail est déjà associé à un autre profil.")
        return existing_email

    existing_pseudo = find_user_by_pseudo(pseudo)
    if existing_pseudo:
        raise ValueError("Ce pseudo est déjà utilisé. Choisis-en un autre.")
    ...
    try:
        ...
        # Les contraintes UNIQUE PostgreSQL restent la source de vérité en cas de concurrence.
        raise ValueError("Ce profil existe déjà. Vérifie l'e-mail et le pseudo.") from exc

    user = _extract_first_row(response)
    if not user:
        raise RuntimeError("Le profil a été créé mais n'a pas pu être chargé.")
    return user


def login_by_identity(email: str, pseudo: str) -> dict[str, Any]:
    email = normalize_email(email)
    pseudo = normalize_pseudo(pseudo)
    if not is_valid_email(email):
        raise ValueError("Saisis une adresse e-mail valide.")
    if not is_valid_pseudo(pseudo):
        raise ValueError("Pseudo invalide : 3 à 24 caractères, lettres/chiffres/_/.- uniquement.")

    user = find_user_by_email(email)
    if not user:
        raise ValueError("Aucun profil associé à cette adresse e-mail.")
    if normalize_pseudo(str(user.get("pseudo", ""))) != pseudo:
        raise ValueError("Les informations saisies ne correspondent pas au profil.")
    return user
...
def save_progress(user_id: str, data: dict[str, Any]) -> None:
    """Actualise la progression du profil dans Supabase."""
    if not user_id:
...
def save_current_progress() -> None:
    ...
    try:
        ...
        st.warning(f"La progression n'a pas pu être synchronisée : {exc}")
...
def render_auth_screen() -> None:
    ...
    # Hero + formulaire : composition inspirée d'un produit SaaS moderne.
    with st.container(key="auth_hero"):
        st.markdown(
            f"""<div class="auth-shell">
                <div class="auth-copy">
                    <div class="brand-lockup">{ROBOT_SVG}<div><div class="brand-name">FRANTSAY</div><div class="brand-kicker">APPRENDRE · PRATIQUER · PROGRESSER</div></div></div>
                    <div class="auth-copy-body">
                        <div class="auth-kicker">Ta nouvelle façon d'apprendre</div>
                        <h1 class="auth-main-title">Apprendre le français<br><span>autrement.</span></h1>
                        <p class="auth-main-sub">Une plateforme pensée pour apprendre à ton rythme, pratiquer avec intelligence et voir tes progrès prendre forme.</p>
                    </div>
                    <div class="auth-feature-grid">
                        <div class="auth-feature"><div class="feature-icon">01</div><b>Apprentissage</b><span>Leçons adaptées</span></div>
                        <div class="auth-feature"><div class="feature-icon green">02</div><b>Missions</b><span>Situations utiles</span></div>
                        <div class="auth-feature"><div class="feature-icon amber">03</div><b>Prononciation</b><span>Travail de l'oral</span></div>
                        <div class="auth-feature"><div class="feature-icon red">04</div><b>Quiz</b><span>Défis rapides</span></div>
                    </div>
                    <div class="auth-quote"><span>“</span><div>La langue est un pont entre les cultures.<br>Franchis-le avec FRANTSAY.</div></div>
                </div>
                <div class="auth-panel">
                    <div class="auth-panel-top"><div><div class="auth-welcome">Bienvenue !</div><div class="auth-welcome-sub">Ton espace d'apprentissage t'attend.</div></div><div class="secure-pill"><span class="secure-dot"></span> Espace personnel</div></div>
                </div>
            </div>""",
    ...
    if not supabase_ready():
        st.error("La connexion à la base de données n'est pas disponible. Vérifie les Secrets Streamlit.")
        st.stop()

    # Le formulaire est placé juste sous le hero, mais visuellement relié à lui.
    col_left, col_right = st.columns([1.15, 1], gap="large")
    with col_left:
        st.markdown('<div class="auth-form-title">Ton espace personnel</div><div class="auth-form-subtitle">Retrouve ton profil avec ton e-mail et ton pseudo.</div>', unsafe_allow_html=True)
        ...
        with tabs[0]:
            with st.form("login_form"):
                ...
                pseudo_input = st.text_input("Pseudo", autocomplete="username", placeholder="Ton pseudo")
                submitted = st.form_submit_button("Se connecter", use_container_width=True)
            if submitted:
                try:
                    with st.spinner("Ouverture de ton espace..."):
                        user = login_by_identity(email_input or "", pseudo_input or "")
        ...
        with tabs[1]:
            with st.form("registration_form"):
                email_input = st.text_input("E-mail", autocomplete="email", placeholder="exemple@email.com")
                pseudo_input = st.text_input("Choisir un pseudo", autocomplete="username", placeholder="3 à 24 caractères")
                submitted = st.form_submit_button("Créer mon espace", use_container_width=True)
            if submitted:
                ...
                if not is_valid_email(email_clean):
                    ...
                    try:
                        with st.spinner("Préparation de ton espace..."):
                            user = create_or_open_account(email_clean, pseudo_clean)
                        restore_user_to_session(user)
                        st.success("Bienvenue dans FRANTSAY.")
                        st.rerun()
                    except Exception as exc:
                        st.error(str(exc))
    ...
    with col_right:
        st.markdown(
            ...
            '<div class="side-card-title">Un parcours qui te ressemble.</div>'
            '<p>Grammaire, missions, prononciation et quiz sont réunis dans un seul espace.</p>'
            '<div class="side-check"><span>✓</span> Profil personnel</div>'
            '<div class="side-check"><span>✓</span> Progression restaurée automatiquement</div>'
            '<div class="side-check"><span>✓</span> Parcours adapté à ton niveau</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            '<div class="identity-note"><span class="identity-note-icon">◎</span><div><b>Ton identité FRANTSAY</b><br><span>Un profil unique relie ton e-mail, ton pseudo et ton parcours.</span></div></div>',
            unsafe_allow_html=True,
        )
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-profile"><div class="profile-avatar">' + safe_html((st.session_state.user_pseudo or "F")[:1].upper()) + '</div><div><b>' + safe_html(st.session_state.user_pseudo or "Profil") + '</b><span>' + safe_html(st.session_state.level) + '</span></div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="sidebar-section-label">NIVEAU</div>', unsafe_allow_html=True)
    level = st.selectbox("Niveau", LEVELS, index=LEVELS.index(st.session_state.level), label_visibility="collapsed")
...
<h1 class="hero-title">Bonjour, {safe_html(st.session_state.user_pseudo)} <span class="wave">✦</span></h1>
<p class="hero-sub">Prêt à continuer ton apprentissage ? · Niveau {safe_html(level)}</p></div><div class="hero-user-badge"><span class="hero-avatar">{safe_html((st.session_state.user_pseudo or 'F')[:1].upper())}</span><div><b>{safe_html(st.session_state.user_pseudo)}</b><span>{safe_html(st.session_state.user_email)}</span></div></div></div>
